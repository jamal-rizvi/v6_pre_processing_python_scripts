...
df = pd.read_excel(os.path.join(gorilla_data, 'igt_control_test.xlsx'))
...
df = df[["Participant Private ID", "Task Name", "Display", "Screen", "Trial Number", "Response", "Spreadsheet: block", "Store: participant_display", "Store: worse", "Store: better"]]
...
df.to_excel('igt_control_pre_processed_part1.xlsx', index=False)

# Read the Excel file into a new variable
filtered_data = os.path.dirname(os.path.abspath(__file__))
df2 = pd.read_excel(os.path.join(filtered_data, 'igt_control_pre_processed_part1.xlsx'))
...
# Save the updated DataFrame to a new Excel file
df2.to_excel('igt_control_pre_processed_part2.xlsx', index=False)

# Read the Excel file into a variable
summery_data = os.path.dirname(os.path.abspath(__file__))
df3 = pd.read_excel(os.path.join(summery_data, 'igt_control_pre_processed_part2.xlsx'))
...
summary_df.to_excel('igt_control_summary.xlsx', index=False)