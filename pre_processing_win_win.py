...
df = pd.read_excel(os.path.join(gorilla_data, 'win_win_test.xlsx'))
...
# "DV: myopic choice" will sum the values from "DV: myopic_choice_2" and "DV: myopic_choice_3"
# "DV: prospective choice" will sum the values from "DV: prospective_choice_1" and "DV: prospective_choice_4"
# "DV: myopic wins" will sum the values from "DV: myopic_wins_2" and "DV: myopic_wins_3"
# "DV: prospective wins" will sum the values from "DV: prospective_wins_1" and "DV: prospective_wins_4"
df['DV: myopic_choice'] = df['DV: myopic_choice_2'] + df['DV: myopic_choice_3']
df['DV: prospective_choice'] = df['DV: prospective_choice_1'] + df['DV: prospective_choice_4']
df['DV: myopic_wins'] = df['DV: myopic_wins_2'] + df['DV: myopic_wins_3']
df['DV: prospective_wins'] = df['DV: prospective_wins_1'] + df['DV: prospective_wins_4']
...
df.to_excel('win_win_pre_processed_part1.xlsx', index=False)

# Read the Excel file into a variable
filtered_data = os.path.dirname(os.path.abspath(__file__))
df2 = pd.read_excel(os.path.join(filtered_data, 'win_win_pre_processed_part1.xlsx'))
...
# Exclude columns titled pre_1	bonus_1	result_1	pre_2	bonus_2	result_2	pre_3	bonus_3	result_3	pre_4	bonus_4	result_4	myopic_choice_2	myopic_choice_3	both_mc	prospective_choice_1	prospective_choice_4	both_pc	myopic_wins_2	myopic_wins_3	both_mw	prospective_wins_1	both_pw	both_pw_mc	prospective_wins_4	prospective_persistence_1	prospective_persistence_4	myopic_persistence_2	myopic_persistence_3	prospective_reinforcement_1	prospective_reinforcement_4	myopic_reinforcement_2	myopic_reinforcement_3	prospective_curiosity_1	prospective_curiosity_4	myopic_curiosity_2	myopic_curiosity_3	prospective_switch_1	prospective_switch_4	myopic_switch_2	myopic_switch_3	both_pp	both_mp	both_pr	both_mr	both_pcu	both_mcu	both_ma	both_pa	both_psw	both_msw	myopic_action_2	myopic_action_3	prospective_action_1	prospective_action_4	myopic_exploit	myopic_explore	prospective_exploit	prospective_explore	total_explore	total_exploit	myopic_model_free	myopic_model_based	prospective_model_free	prospective_model_based	total_model_free	total_model_based
df2 = df2.drop(columns=['Screen', 'Display', 'display', 'Tag', 'A_PRE', 'A_POST', 'B_PRE', 'B_POST', 'C_PRE', 'C_POST', 'D_PRE', 'D_POST', 'A_RESULT', 'B_RESULT', 'C_RESULT', 'D_RESULT',
                       'pre_1', 'bonus_1', 'result_1', 'pre_2', 'bonus_2', 'result_2', 'pre_3', 'bonus_3', 'result_3', 'pre_4', 'bonus_4', 'result_4', 'myopic_choice_2', 'myopic_choice_3', 
                       'both_mc', 'prospective_choice_1', 'prospective_choice_4', 'both_pc', 'myopic_wins_2', 'myopic_wins_3', 'both_mw', 'prospective_wins_1', 'both_pw', 'both_pw_mc', 
                       'prospective_wins_4', 'prospective_persistence_1', 'prospective_persistence_4', 'myopic_persistence_2', 'myopic_persistence_3', 'prospective_reinforcement_1', 
                       'prospective_reinforcement_4', 'myopic_reinforcement_2', 'myopic_reinforcement_3', 'prospective_curiosity_1', 'prospective_curiosity_4', 'myopic_curiosity_2', 
                       'myopic_curiosity_3', 'prospective_switch_1', 'prospective_switch_4', 'myopic_switch_2', 'myopic_switch_3', 'both_pp', 'both_mp', 'both_pr', 'both_mr', 'both_pcu', 
                       'both_mcu', 'both_ma', 'both_pa', 'both_psw', 'both_msw', 'myopic_action_2', 'myopic_action_3', 'prospective_action_1', 'prospective_action_4', 'myopic_exploit', 
...
# Save the updated DataFrame to a new Excel file
df2.to_excel('win_win_pre_processed_part2.xlsx', index=False)

# Read the Excel file into a variable
summery_data = os.path.dirname(os.path.abspath(__file__))
df3 = pd.read_excel(os.path.join(summery_data, 'win_win_pre_processed_part2.xlsx'))
...
summary_df.to_excel('win_win_summary.xlsx', index=False)